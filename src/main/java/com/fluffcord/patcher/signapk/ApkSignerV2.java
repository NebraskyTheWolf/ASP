package com.fluffcord.patcher.signapk;
...
public abstract class ApkSignerV2 {
    ...
    public static ByteBuffer[] sign(
        ...
        ((Buffer)inputApk).position(eocdOffset);
        ...
        ((Buffer)originalInputApk).position(originalInputApk.limit());
        ...
        ((Buffer)beforeCentralDir).clear();
        ((Buffer)centralDir).clear();
        ((Buffer)eocd).clear();
    ...
    private static ByteBuffer getByteBuffer(ByteBuffer source, int size) {
        ...
        ((Buffer)source).limit(limit);
        try {
            ByteBuffer result = source.slice();
            result.order(source.order());
            ((Buffer)source).position(limit);
            return result;
        } finally {
